...
int main()
{
    ...
    vector<Eigen::Isometry3d> poses;    //4 x 4 T matrix

    ifstream fin("../pose.txt");
    if(!fin)
    {
        cout << "Please run the program with pose.txt!" << endl;
        return 1;
    }

    for(int i =0; i < 5; i++)
    {
        boost::format fmt("../%s/%d.%s");  //format of image
    ...
    cout << "Transforming the image to PointCLoud..." << endl;
    ...
    for (int i=0; i < 5; i++)
    {
        cout << "transform the image " << i+1 << endl;
