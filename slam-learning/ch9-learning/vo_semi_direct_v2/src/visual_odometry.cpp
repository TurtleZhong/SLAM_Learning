...
bool VisualOdometry::addFrame(Frame::Ptr frame, string Method)
{
    ...
    case OK:
    {
        ...
        selectCandidates();            /*prepare for estimate*/
...
{
    /*add key-Frame suscess!!*/
    cout << "add key-Frame suscess!!" << endl;
    if ( map_->keyframes_.empty() )
    {
        ...
        for ( int i=0; i<measurements_curr_.size(); i++ )
        {
            ...
            MapPoint::Ptr map_point = MapPoint::createMapPoint(
                        measurements_curr_[i].pos_world, n, measurements_curr_[i].grayscale, curr_.get()
                        );
...
void VisualOdometry::selectCandidates()
{
    // select the candidates in map
    for ( auto& allpoints: map_->map_points_ )
    {
...
void VisualOdometry::poseEstimationDirect()
{
    ...
    // 添加边
    int id=1;
    for ( MapPoint::Ptr m: candidate_  ) //Measurement m: measurements_curr_
    {
        ...
        edge->setVertex ( 0, pose );
        edge->setMeasurement ( m->gray_value_ );
    ...
    for ( MapPoint::Ptr m: candidate_ )
    {
        if ( rand() > RAND_MAX/5 )
            continue;
        Eigen::Vector3d p = m->pos_;
        //Eigen::Vector2d pixel_prev = project3Dto2D ( p ( 0,0 ), p ( 1,0 ), p ( 2,0 ), fx, fy, cx, cy );
        Eigen::Vector2d pixel_prev = curr_->camera_->camera2pixel(p);
        //Eigen::Vector2d pixel_prev = curr_->camera_->world2pixel(p, ref_->T_c_w_ );
        Eigen::Vector3d p2 = T_c_w_estimated_*m->pos_;
